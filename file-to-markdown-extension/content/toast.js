...
// ===========================================================================
// The auto-skip countdown is a compositor-driven CSS animation plus a single
// expiry timer, replacing a 20 Hz setInterval that wrote style.width and
// forced ~200 layout invalidations on the host page.
// ===========================================================================
...
(() => {
  ...
  class Toast {
    ...
    create() {
      ...
      this.root.appendChild(el('style')).textContent = FTM.toastStyles();
    ...
    show(file, extraCount) {
      ...
      n.hint.textContent = extraCount > 0
        ? 'Enter = convert \u00B7 Esc = skip \u00B7 +' + extraCount + ' more file' + (extraCount > 1 ? 's' : '') + ' kept as-is'
        : 'Enter = convert \u00B7 Esc = skip';
      n.approve.disabled = false;
      n.approve.textContent = 'Convert';
      n.approve.style.opacity = '';
  ...
  FTM.toastStyles = function toastStyles() {
    return `
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* Compositor-only countdown: no per-frame JavaScript, no layout. */
