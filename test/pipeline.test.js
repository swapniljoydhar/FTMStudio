...
test('binary transport frames a file as BEGIN / CHUNK* / END', async () => {
  ...
  const file = { name: 'a.docx', size: bytes.length, arrayBuffer: async () => bytes.buffer };
...
test('transport rejects oversized files before reading them', async () => {
  ...
  const file = { name: 'huge.pdf', size: FTM.CONSTANTS.MAX_FILE_SIZE_BYTES + 1, arrayBuffer: async () => { throw new Error('must not read'); } };
...
test('transport surfaces offscreen errors', async () => {
  ...
  const file = { name: 'a.pdf', size: 4, arrayBuffer: async () => new Uint8Array([1, 2, 3, 4]).buffer };
